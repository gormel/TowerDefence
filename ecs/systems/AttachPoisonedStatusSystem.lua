...
---@class AttachPoisonedStatusSystem : ecstasy.System
local AttachPoisonedStatusSystem = ecstasy.System("AttachPoisonedStatusSystem")

function AttachPoisonedStatusSystem:init()
    self.status_links = self.world:get_table(Components.HasPoisonedStatus)
	...
	self.statuses = self.world:get_table(Components.PoisonedStatus)
	self.poisoneds = self.world:get_table(Components.Poisoned)
	self.destroyeds = self.world:get_table(Components.Destroyed)
    
    self.add_filter = self.world:create_filter(Components.Poisoned, exc(Components.HasPoisonedStatus))
    self.del_filter = self.world:create_filter(Components.HasPoisonedStatus, exc(Components.Poisoned))
end

function AttachPoisonedStatusSystem:execute()
    for _, entity in self.add_filter:reverse_entities() do
        local poisoned = self.poisoneds:get(entity)
        local status_cfg = setup.Statuses[poisoned.status_id]
...
return AttachPoisonedStatusSystem